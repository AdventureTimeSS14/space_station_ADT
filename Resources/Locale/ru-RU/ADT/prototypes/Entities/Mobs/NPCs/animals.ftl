ent-ADTMobPigeon = голубь
    .desc = Те, кто в городе больше всего докучают... Теперь и в космосе!
ent-ADTModTurtle = черепаха
    .desc = Морская черепашка, по имени Наташка. На этом закончили.
ent-ADTMobDragonPet = детёныш дракона
    .desc = Даже крылатый левиафан когда-то начинал с яйца.
ent-ADTModAntlion = муравьиный лев
    .desc = Естественный враг комбайных машин. Не любит вибрации.
ent-ADTMobGondola = гондола
    .desc = Его нейтральное выражение лица показывает принятие мира таким, какой он есть.
ent-ADTMobNotariusSpider = Нотариус
    .desc = Бюрократически подкованный паук, который будет вам помогать. Удивительно любит церковь!
ent-ADTChurchMobCarp = { ent-MobCarp }
    .desc = { ent-MobCarp.desc }
ent-ADTMobMouseSpace = космическая мышь
    .desc = Грызун, покоряющий космос.
ent-ADTMobMonkeyAdvanced = обезьяна
    .desc = вам кажется эта обезьяна немного лучше других владеет своими конечностями, чем её собратья
    .suffix = Продвинутый
ent-ADTMobMonkeyAdvanced = прото-сохун
    .desc = вам кажется этот прото-сохун немного лучше других владеет своими конечностями, чем его менее разумные собратья
    .suffix = Продвинутый
