ent-ADTSpaceSecWall = укреплённая стена
    .desc = Удерживает воздух внутри, а дронов снаружи.
ent-ADTSpaceSecWallDiagonal = диагональная укреплённая стена
    .desc = Удерживает воздух внутри, а дронов снаружи.
ent-ADTCardboardWall = картонная стена
    .desc = Стена из бумаги и тонких досок. Пользуется определенной популярностью в [восточном месте].
ent-ADTStonebrickWall = каменная стена
    .desc = Классическая каменная стена.
ent-ADTYellowbrickWall = кирпичная жёлтая стена
    .desc = Желтые кирпичи, уложенные по древним обычаям.

ent-ADTAsteroidRockIndestructible = астероидный камень
    .desc = Каменистый астероид.
    .suffix = Неразрушимый

ent-ADTWallRockAndesiteIndestructible = андезит
    .desc = Стена из андезита.
    .suffix = Неразрушимый

ent-ADTWallRockBasaltIndestructible = базальт
    .desc = Стена из базальта.
    .suffix = Неразрушимый

ent-ADTWallRockSnowIndestructible = сугроб
    .desc = Стена из снега.
    .suffix = Неразрушимый

ent-ADTWallRockSandIndestructible = песчаник
    .desc = Стена из песка.

ent-ADTWallRockIndestructible = камень
    .desc = Каменная стена.


ent-ADTIronRockIndestructible = камень
    .desc = Каменная стена.

ent-ADTDigitalWall = цифровая стена
    .desc = 

ent-ADTDigitalRedWall = красная цифровая стена
    .desc= 

ent-ADTDigitalBlueWall = синяя цифровая стена
    .desc = 0110101100010000011000100001010000111110100001110101011001000001000011100001101101000011010110001000001101000001000011011010000110101100010000

ent-ADTDigitalWallIndestructible = цифровая стена
    .desc = { ent-ADTDigitalWall.desc }
    .suffix = Неразрушимый

ent-ADTDigitalRedWallIndestructible = красная цифровая стена
    .desc = { ent-ADTDigitalRedWall.desc }
    .suffix = Неразрушимый

ent-ADTDigitalBlueWallIndestructible = синяя цифровая стена
    .desc = { ent-ADTDigitalBlueWall.desc }
    .suffix = Неразрушимый

ent-ADTRadiationUraniumWall = урановая стена
    .desc = Вам кажется, или эта стена, сделанная из чистого урана, светится?
    .suffix = Неразрушимый

ent-ADTAbductorWall = инопланетная стена
    .desc = Удерживает серых человечков внутри, а скот - снаружи.
    .suffix = Неразрушимый

ent-ADTAbductorWallDiagonal = диагональная инопланетная стена
    .desc = Удерживает серых человечков внутри, а скот - снаружи.
    .suffix = Неразрушимый, диагональный

ent-ADTWallSolidWhite = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, белая
ent-ADTWallSolidBlack = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, чёрная
ent-ADTWallSolidPurple = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, фиолетовая
ent-ADTWallSolidRed = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, красная
ent-ADTWallSolidBlue = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, синяя
ent-ADTWallSolidTeal = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, аквамариновая
ent-ADTWallSolidBrown = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, коричневая
ent-ADTWallSolidGray = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, серая
ent-ADTWallSolidGreen = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, зелёная
ent-ADTWallSolidLightBrown = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, светло-коричневая
ent-ADTWallSolidOrange = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, оранжевая
ent-ADTWallSolidYellow = { ent-WallSolid }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Цветная, жёлтая

ent-ADTWallAdminWhite = абсолютно белая стена
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Адмемы
ent-ADTWallAdminWhiteUnshaded = { ent-ADTWallAdminWhite }
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Адмемы, unshaded
ent-ADTWallAdminBlack = абсолютно чёрная стена
    .desc = { ent-BaseStructureWall.desc }
    .suffix = Адмемы
