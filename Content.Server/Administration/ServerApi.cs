...
public sealed partial class ServerApi : IPostInjectInit
{
    ...
    private async Task AdminConsoleAction(IStatusHandlerContext context, Actor actor)
    {
        ...
        if (Guid.TryParse(actor.Guid.ToString(), out var guid))
        {
            var username = new NetUserId(guid);
            var sessionCustom = _playerManager.CreateAndAddSession(username, actor.Name);
        ...
        await RespondOk(context);
        // _playerManager.RemoveSession(username);
    }
