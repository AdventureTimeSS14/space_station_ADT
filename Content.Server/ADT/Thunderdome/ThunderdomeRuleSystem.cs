...
public sealed partial class ThunderdomeRuleSystem : EntitySystem
{
    ...
    public void SpawnPlayer(ICommonSession session, EntityUid ruleEntity, int weaponIdx)
    {
        ...
        if (mindComp.UserId is { } userId && _prefs.TryGetCachedPreferences(userId, out var prefs))
            profile = (prefs.SelectedCharacter as HumanoidCharacterProfile)?.WithSpecies(HumanoidCharacterProfile.DefaultSpecies);
        ...
        if (ghostEntity != null && Exists(ghostEntity))
        {
    ...
    private void CreditKill(Entity<ThunderdomePlayerComponent> victim, ThunderdomeRuleComponent rule, EntityUid? killer = null)
    {
        ...
        // лечащий тулбокс
        var killerTransform = Transform(killerUid);
        if (killerTransform.Coordinates != null)
        {
            Spawn("HealingToolbox", killerTransform.Coordinates);
        }
    }
