...
public sealed partial class HereticAbilitySystem : SharedHereticAbilitySystem
{
    ...
    private void OnLivingHeartActivate(EventHereticLivingHeartActivate args)
    {
        ...
        else
        {
            ...
            if (isOnStation && distance <= 50f)
                _aud.PlayGlobal(new SoundPathSpecifier("/Audio/ADT/Heretic/heartbeat.ogg"), target.Value, AudioParams.Default.WithVolume(-distance * 0.5f)); // steep dropoff so it's more impactful when they're close
        }
    ...
    {
        EnsureComp<ActiveRadioComponent>(target).Channels.Add(MansusLinkChannel);
        EnsureComp<IntrinsicRadioReceiverComponent>(target);
        EnsureComp<IntrinsicRadioTransmitterComponent>(target).Channels.Add(MansusLinkChannel);
    }

    public bool HasMansusLink(EntityUid target)
    {
        return TryComp<IntrinsicRadioTransmitterComponent>(target, out var transmitter) &&
               transmitter.Channels.Contains(MansusLinkChannel);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        while (fleshQuery.MoveNext(out var uid, out var flesh, out var modifiers, out var dmg))
        {
            ...
            IHateWoundMed((uid, dmg, null), toHeal, boneHeal, painHeal, woundHeal, bloodHeal, bleedHeal);
        }
