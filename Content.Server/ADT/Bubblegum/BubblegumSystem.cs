...
public sealed class BubblegumSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<BubblegumComponent, MegafaunaSpokeEvent>(OnSpoke);
    }
    ...
    private void OnMobStateChanged(Entity<BubblegumComponent> ent, ref MobStateChangedEvent args)
    {
        ...
        _megafauna.Say(ent, "bubblegum-death", 3, true);
    ...
    private void OnDamageDealt(Entity<BubblegumComponent> ent, ref DamageDealtEvent args)
    {
        ...
        if (TryComp<MobThresholdsComponent>(ent, out var thresholds))
        {
            ...
            if (!ent.Comp.InSmashPhase && damageTaken >= maxHealth * 0.5f)
            {
                ...
                _megafauna.Say(ent, "bubblegum-smash-phase", 3, true);
            }
        }
    ...
    private void OnSpoke(Entity<BubblegumComponent> ent, ref MegafaunaSpokeEvent args)
    {
        var minionQuery = EntityQueryEnumerator<BubblegumMinionComponent>();

        while (minionQuery.MoveNext(out var minionUid, out var minionComp))
        {
            if (minionComp.Summoner != ent.Owner)
                continue;

            _megafauna.SpeakLine(minionUid, args.Message);
        }
    }
}
