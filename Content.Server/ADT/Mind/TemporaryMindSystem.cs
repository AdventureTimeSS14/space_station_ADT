...
public sealed partial class TemporaryMindSystem : EntitySystem
{
    ...
    public bool TrySwapTempMind(ICommonSession session, EntityUid newBody)
    {
        ...
        {
            Log.Warning($"TrySwapTempMind FAIL: no AttachedEntity, session={session}");
            return false;
        }

        if (!_mind.TryGetMind(currentEntity, out var origMindId, out var origMind))
        {
            Log.Warning($"TrySwapTempMind FAIL: no Mind on {currentEntity}, session={session}");
            return false;
        }
    ...
    private void CleanupDisposableMind(TemporaryMindComponent temp)
    {
            ...
            && origMind.OriginalOwnerUserId is { } userId)
        {
            ...
            // Recompute the examine state now that the original mind owns the body again
            if (origMind.OwnedEntity is { } originalBody)
                _mindEx.RefreshMindStatus(originalBody);
        }
    }
}
