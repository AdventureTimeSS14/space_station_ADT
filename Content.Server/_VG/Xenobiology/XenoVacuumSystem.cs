using Content.Shared._VG.Xenobiology.Components.Equipment;
...
namespace Content.Server._VG.Xenobiology;
...
public sealed partial class XenoVacuumSystem : EntitySystem
{
    ...
    private void OnAfterInteract(Entity<XenoVacuumComponent> ent, ref AfterInteractEvent args)
    {
        ...
        foreach (var removedEnt in _containerSystem.EmptyContainer(tankComp.StorageTank))
        {
            ...
            // Исправлено: используем GetEntity чтобы получить Entity<HTNComponent>
            if (TryComp<HTNComponent>(removedEnt, out var htn))
                _htn.SetHTNEnabled((removedEnt, htn), true, 2f);
        }
