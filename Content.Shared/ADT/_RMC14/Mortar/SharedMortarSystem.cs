...
public abstract class SharedMortarSystem : EntitySystem
{
    ...
    private void OnMortarTargetDoAfter(Entity<MortarComponent> mortar, ref TargetMortarDoAfterEvent args)
    {
        ...
        if (_mapManager.TryFindGridAt(_transform.GetMapCoordinates(mortar.Owner), out var uid, out var grid))
        {
            var gridCenterWorld = grid.LocalAABB.Center;
            offset -= new Vector2i((int)gridCenterWorld.X, (int)gridCenterWorld.Y);
        }
    ...
    private void OnMortarLoadDoAfter(Entity<MortarComponent> mortar, ref LoadMortarShellDoAfterEvent args)
    {
        ...
        _adminLogs.Add(LogType.AdminMessage, LogImpact.High, $"Mortar {ToPrettyString(mortar)} shell {ToPrettyString(shellId)} shot by {ToPrettyString(user)} aimed at {coordinates}");
