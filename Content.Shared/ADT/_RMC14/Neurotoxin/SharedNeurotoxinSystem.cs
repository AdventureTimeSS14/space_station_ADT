...
using Content.Shared._RMC14.GasToggle;
...
public abstract class SharedNeurotoxinSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (neurotoxinInjectorQuery.MoveNext(out var uid, out var neuroGas))
        {
            ...
            _humanoid.Clear();
            _entityLookup.GetEntitiesInRange(uid.ToCoordinates(), 0.5f, _humanoid);
            foreach (var humanoid in _humanoid)
            {
                if (!neuroGas.AffectsDead && _mobState.IsDead(humanoid))
                    continue;

                if (!EnsureComp<NeurotoxinComponent>(humanoid, out var builtNeurotoxin))
                {
