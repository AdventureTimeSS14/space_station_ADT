...
using Content.Shared.Flash.Components; // ADT-Tweak
...
public abstract partial class SharedAugmentedEyesightSystem : EntitySystem
{
    ...
    private void OnShutdown(Entity<AugmentedEyesightComponent> ent, ref ComponentShutdown args)
    {
        ...
        SetVision(ent, true);
        RemComp<FlashModifierComponent>(ent);
    }
    ...
    private void UpdateFlashVulnerability(Entity<AugmentedEyesightComponent> ent)
    {
        if (ent.Comp.Enabled)
        {
            RemComp<FlashModifierComponent>(ent);
