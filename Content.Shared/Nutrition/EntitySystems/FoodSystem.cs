...
public sealed class FoodSystem : EntitySystem
{
    ...
    public (bool Success, bool Handled) TryFeed(EntityUid user, EntityUid target, EntityUid food, FoodComponent foodComp)
    {
        ...
        // ADT Quirks start
        var delay = foodComp.Delay;
        if (TryComp<FoodConsumptionSpeedModifierComponent>(user, out var modifier))
            delay *= modifier.Modifier;
        // ADT Quirks end

        var doAfterArgs = new DoAfterArgs(EntityManager,
            user,
            forceFeed ? foodComp.ForceFeedDelay : delay,    // ADT Quirks tweak
    ...
    private void OnDoAfter(Entity<FoodComponent> entity, ref ConsumeDoAfterEvent args)
    {
        ...
        args.Repeat = false;
        DeleteAndSpawnTrash(entity.Comp, entity.Owner, args.Target.Value); // ADT-Tweak
    }
