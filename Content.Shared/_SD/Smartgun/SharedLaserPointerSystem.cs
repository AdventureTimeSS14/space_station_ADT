...
public abstract class SharedLaserPointerSystem : EntitySystem
{
    ...
    private void OnHovered(LaserPointerEntityHoveredEvent msg, EntitySessionEventArgs args)
    {
        ...
        // Проверяем, наведены ли на NPC
        bool isLockingOnNpc = hovered != null && HasComp<NpcFactionMemberComponent>(hovered);

        // Если только что навелись на NPC (были не на NPC, а теперь на NPC)
        if (isLockingOnNpc && laser.CurrentLockedTarget != hovered)
        {
            _audio.PlayPredicted(laser.LockOnSound, pointer, pointer);
            laser.CurrentLockedTarget = hovered;
        }
        // Если ушли с NPC (были на NPC, а теперь не на NPC)
        else if (!isLockingOnNpc && laser.CurrentLockedTarget != null)
        {
            laser.CurrentLockedTarget = null;
        }
    ...
    {
        ent.Comp.CurrentLockedTarget = null;
        RemoveLine(GetNetEntity(ent.Owner));
    }
