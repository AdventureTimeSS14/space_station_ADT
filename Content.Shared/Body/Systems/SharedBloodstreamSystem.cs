...
public abstract class SharedBloodstreamSystem : EntitySystem
{
    ...
    [Dependency] private readonly SharedHushedSystem _hushedSystem = default!; //ADT-Tweak
        ...
        while (query.MoveNext(out var uid, out var bloodstream))
        {
            ...
            if (!_mobStateSystem.IsDead(uid))
            {
                ...
                if (bloodPercentage < bloodstream.BloodlossThreshold)
                {
                    ...
                    _hushedSystem.DoHushed(uid, bloodstream.AdjustedUpdateInterval * 2, refresh: false); // ADT-Tweak
                }
                else
                {
                    ...
                    _hushedSystem.DoRemoveHushed(uid); // ADT-Tweak
                }
            }
            else
            {
