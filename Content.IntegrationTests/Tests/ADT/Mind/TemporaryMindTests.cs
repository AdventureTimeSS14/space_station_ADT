...
namespace Content.IntegrationTests.Tests.ADT.Mind;

[TestFixture]
public sealed partial class TemporaryMindTests
{
    // ADT: test body prototype — ghostOnShutdown is set declaratively because
    // integration tests are not allowed to write to MindContainerComponent directly (RA0002).
    [TestPrototypes]
    private const string Prototypes = @"
- type: entity
  id: TemporaryMindTestBody
  components:
  - type: MindContainer
    ghostOnShutdown: false
";
    ...
    private static async Task<TestContext> SetupContext(TestPair pair)
    {
        ...
        await server.WaitPost(() =>
        {
            ...
            ctx.OriginalBody = entMan.SpawnEntity("TemporaryMindTestBody", MapCoordinates.Nullspace);
            mindSys.TransferTo(ctx.OrigMindId, ctx.OriginalBody);
            playerMan.SetAttachedEntity(player, ctx.OriginalBody);

            ctx.NewBody = entMan.SpawnEntity("TemporaryMindTestBody", MapCoordinates.Nullspace);
        });
    ...
    private sealed class TestContext
    {
        ...
        public MindSystem MindSys = default!;
