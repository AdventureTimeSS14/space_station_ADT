    ...
    public async Task TestSuicideByHeldItem()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(entManager.TryGetComponent<GhostComponent>(mindComponent.CurrentEntity, out var ghostComp) &&
                            !ghostComp.CanReturnToBody);
                Assert.That(damageableComp.Damage.GetTotal(), Is.GreaterThanOrEqualTo(lethalDamageThreshold));  // ADT Fix
            });
        });
    ...
    public async Task TestSuicideByHeldItemSpreadDamage()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(entManager.TryGetComponent<GhostComponent>(mindComponent.CurrentEntity, out var ghostComp) &&
                            !ghostComp.CanReturnToBody);
                Assert.That(damageableComp.Damage.GetTotal(), Is.GreaterThanOrEqualTo(lethalDamageThreshold / 2));
            });
        });
