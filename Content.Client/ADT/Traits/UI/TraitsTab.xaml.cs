...
public sealed partial class TraitsTab : BoxContainer
{
    ...
    public TraitsTab()
    {
        ...
        _cfg.OnValueChanged(SimpleStationCCVars.MaxTraitPoints, OnMaxTraitPointsChanged, true);
    ...
    public void UpdateRequirements(HumanoidCharacterProfile? profile, ProtoId<JobPrototype>? jobId = null)
    {
        // Note: jobId is typically null in the character editor since the job is assigned server-side on spawn.
        // Department blacklist/whitelist checks will be performed server-side.
        foreach (var (_, categoryUi) in _categoryUis)
            categoryUi.UpdateConditions(profile, jobId);
