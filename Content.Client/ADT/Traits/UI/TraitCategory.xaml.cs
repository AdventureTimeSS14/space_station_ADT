...
public sealed partial class TraitCategory : BoxContainer
{
    ...
    public void SetTraitSelected(ProtoId<TraitPrototype> traitId, bool selected, bool suppressToggle = false)
    {
        if (_traitEntries.TryGetValue(traitId, out var entry))
            entry.SetSelected(selected, suppressToggle);
    }
    ...
    public void FilterTraits(string searchText)
    {
        ...
        foreach (var (traitId, entry) in _traitEntries)
        {
            ...
            var description = Loc.GetString(trait.Description ?? string.Empty);
    ...
    /// </summary>
    public void UpdateConditions(HumanoidCharacterProfile? profile, ProtoId<JobPrototype>? jobId = null)
    {
        foreach (var (_, entry) in _traitEntries)
            entry.UpdateConditionsMet(profile, jobId);
