...
using Robust.Client.Input; // ADT tweak
using Robust.Client.Player; // ADT tweak
...
public sealed partial class LatheMenu : FancyWindow
{
    ...
    [Dependency] private readonly IInputManager _input = default!; // ADT tweak
    [Dependency] private readonly IPlayerManager _player = default!; // ADT tweak
    [Dependency] private readonly IPrototypeManager _prototypeManager = default!;

    private readonly SpriteSystem _spriteSystem;
    private readonly LatheSystem _lathe;
    private readonly MaterialStorageSystem _materialStorage;
    private readonly MiningPointsSystem _miningPoints; // ADT tweak
    ...
    public event Action? OnClaimMiningPoints; // ADT tweak
    public event Action<string>? SmeltAllAction; // ADT tweak
    ...
    public LatheMenu()
    {
        ...
        _miningPoints = _entityManager.System<MiningPointsSystem>(); // ADT tweak
    ...
    public void SetEntity(EntityUid uid)
    {
        ...
        // ADT tweak start: Mining points UI
    ...
    /// <summary>
    /// ADT tweak: Updates the UI elements for mining points.
    /// </summary>
    ...
    /// <summary>
    /// ADT tweak: Update mining points UI whenever it changes.
    /// </summary>
    ...
    public void PopulateRecipes()
    {
        ...
        foreach (var prototype in sortedRecipesToShow)
        {
            ...
            canProduce |= GetSmeltAllAmount(prototype.ID) != null;
            var tooltipFunction = () => GenerateTooltipText(prototype);

            if (idx >= oldChildCount)
            {
                var control = new RecipeControl(_lathe, prototype, tooltipFunction, canProduce, GetRecipeDisplayControl(prototype), _entityManager.HasComponent<MiningPointsLatheComponent>(Entity));
                control.OnButtonPressed += s =>
                {
                    if (CanSmeltAll() && _input.IsKeyDown(Keyboard.Key.Shift))
                    {
                        SmeltAllAction?.Invoke(s);
                        return;
                    }
                    // ADT tweak end
    ...
    private bool CanSmeltAll()
    {
        return _entityManager.HasComponent<MiningPointsLatheComponent>(Entity);
    }

    private int? GetSmeltAllAmount(string recipeId)
    {
        if (!CanSmeltAll())
            return null;
        ...
        if (!_prototypeManager.TryIndex<LatheRecipePrototype>(recipeId, out var recipe) || recipe.Materials.Count == 0)
            return null;
        ...
        foreach (var (id, needed) in recipe.Materials)
        {
            ...
            amount = Math.Min(amount, _materialStorage.GetMaterialAmount(Entity, id) / perItem);
        }
    ...
    private string GenerateTooltipText(LatheRecipePrototype prototype)
    {
        ...
        // ADT tweak: tell the player about shift+click
        if (GetSmeltAllAmount(prototype.ID) is { } all)
            sb.AppendLine(Loc.GetString("lathe-menu-smelt-all", ("amount", all)));
