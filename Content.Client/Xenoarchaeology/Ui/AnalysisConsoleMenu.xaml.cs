...
public sealed partial class AnalysisConsoleMenu : FancyWindow
{
    ...
    private void StartExtract(BaseButton.ButtonEventArgs obj)
    {
        ...
        // ADT-Tweak start
        var multiplier = 1f;
        if (_artifactAnalyzer.TryGetAnalyzer(_owner, out var analyzer)
            && _ent.TryGetComponent<ArtifactAnalyzerUpgradeComponent>(analyzer, out var upgrade))
        {
            multiplier = upgrade.PointMultiplier;
        }
        // ADT-Tweak end
        ...
        {
            var pointValue = (int) (_xenoArtifact.GetResearchValue(node) * multiplier); // ADT-Tweak
            if (pointValue <= 0)
                continue;
