...
public sealed partial class StationAILoadoutWindow : BaseLoadoutWindow, ILoadoutOverride
{
    ...
    private void PopulateSkins(IPrototypeManager protoMan, RoleLoadout loadout)
    {
        ...
        var list = protoMan.EnumeratePrototypes<StationAiCustomizationPrototype>()
                            .Where(x => x.PreviewKey == "Occupied")
                            .OrderBy(x => x.Priority).ThenByDescending(x => Loc.GetString(x.Name))
                            .ToList();
        ...
        {
            var layer = item.LayerData["Occupied"];
            ...
            sprite.SetFromSpriteSpecifier(new SpriteSpecifier.Rsi(new(layer.RsiPath), layer.State), new(2, 2));
