...
public sealed partial class CrewMonitoringWindow : BaseWindow   // ADT-Tweak - New Monitor new parent
{
    ...
    public Action<NetEntity>? OnAiEyeTeleport; // ADT-Tweak
    ...
    {
        // ADT-Tweak start
        var isAi = _player.LocalEntity is { } localEntity &&
        ...
        foreach (var sensor in departmentSensors)
        {
            ...
            if (isAi)
            {
                var eyeTeleportButton = new Button
                {
                    Text = Loc.GetString("ai-eye-teleport-button"),
                    ToolTip = Loc.GetString("ai-eye-teleport-hover"),
                    Disabled = sensor.Mode != SuitSensorMode.SensorCords,
                    MinSize = new Vector2(28, 28),
                    Margin = new Thickness(6, 0, 0, 0),
                    StyleBoxOverride = _buttonStyle,
                    HorizontalAlignment = HAlignment.Right,
                    VerticalAlignment = VAlignment.Center,
                };

                eyeTeleportButton.OnPressed += _ => OnAiEyeTeleport?.Invoke(sensor.OwnerUid);
                mainContainer.AddChild(eyeTeleportButton);
            }
            // ADT-Tweak End
